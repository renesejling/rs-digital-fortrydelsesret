...
function rs_fr_generate_pdf() {
	if ( ! class_exists( '\Dompdf\Dompdf' ) ) {
		return false; // Dompdf ikke installeret endnu.
	}

	$terms_id = (int) get_option( 'woocommerce_terms_page_id' );
	if ( ! $terms_id ) {
		return false; // Ingen handelsbetingelses-side valgt i WooCommerce.
	}
	...
	return (bool) file_put_contents( rs_fr_pdf_path(), $dompdf->output() );
}
